...
function Dashboard({ initialEvent }) {
  ...
  const [attendeeOrigin, setAttendeeOrigin] = useState(() => localStorage.getItem('tinkerBingoAttendeeOrigin') || window.location.origin);
  const event = snapshot.event;
  const count = snapshot.attendees.length;
  const proposed = suggestedGrid(count);
  const validOrigin = (() => { try { return new URL(attendeeOrigin).origin; } catch { return window.location.origin; } })();
  const joinUrl = new URL(`/join?event=${event.id}`, validOrigin).toString();
  const updateAttendeeOrigin = (value) => { setAttendeeOrigin(value); localStorage.setItem('tinkerBingoAttendeeOrigin', value); };
  ...
  return <main className="dashboard"><header><div><span className="eyebrow">HOST DASHBOARD</span><h1>{event.name}</h1><p className={`status ${event.status}`}>{event.status}</p></div><button className="quiet" onClick={() => { localStorage.removeItem('tinkerBingoHostEvent'); location.reload(); }}>New event</button></header>
    <div className="dashboard-grid"><section className="card qr-card"><h2>Check-in QR</h2><label className="origin-input">Attendee address<input value={attendeeOrigin} onChange={e => updateAttendeeOrigin(e.target.value)} placeholder="https://your-app.vercel.app" /></label><div className="qr"><QRCodeSVG value={joinUrl} size={220} includeMargin /></div><a href={joinUrl} target="_blank" rel="noreferrer">Open registration link</a><code>{joinUrl}</code></section>
      <section className="card monitor"><h2>Registration monitor</h2><div className="count"><strong>{count}</strong><span>registered</span></div><label>Expected headcount <input type="number" min="0" value={expected} onChange={e => setExpected(e.target.value)} placeholder="Optional" /></label>{missing !== null && <p className="muted">{missing === 0 ? 'Everyone expected is checked in.' : `${missing} more expected.`}</p>}
        <div className="start-panel"><h3>Start game</h3>{event.status === 'registering' ? <>{(import.meta.env.DEV || import.meta.env.VITE_ENABLE_DEMO_SEED === 'true') && <div className="demo-control">{!import.meta.env.DEV && <input type="password" value={demoPassword} onChange={e => setDemoPassword(e.target.value)} placeholder="Demo test passphrase" />}<button className="seed-button" onClick={seedPlayers} disabled={busy}>Add 10 demo players</button></div>}{proposed ? <p>{count} players supports a <b>{proposed}-cell</b> grid.</p> : <p className="warning">Not enough attendees — need at least 11.</p>}<label>Grid size<select value={gridSize ?? ''} onChange={e => setGridSize(Number(e.target.value))} disabled={!proposed}><option value="">Choose size</option><option value="10">10 cells</option><option value="15" disabled={count < 16}>15 cells</option><option value="20" disabled={count < 21}>20 cells</option></select></label><button onClick={start} disabled={!gridSize || busy}>{busy ? 'Starting…' : 'Start game'}</button></> : <p className="live-message">Game is {event.status}. Grid: {event.grid_size} cells.</p>}</div>{testPlayers.length > 0 && <div className="demo-codes"><b>Demo verification codes</b>{testPlayers.map(player => <span key={player.secret_code}>{player.name}: <code>{player.secret_code}</code></span>)}</div>}{notice && <p className="notice">{notice}</p>}</section>
