...
function Dashboard({ initialEvent }) {
  ...
  async function seedPlayers() {
    setBusy(true); setNotice('');
    try { const result = await api(`/dev/events/${event.id}/seed`, { method: 'POST' }); setTestPlayers(result.players); setNotice('Demo players added. Their codes are shown below.'); const data = await api(`/events/${event.id}`); setSnapshot(data); }
    catch (err) { setNotice(err.message); } finally { setBusy(false); }
  }
