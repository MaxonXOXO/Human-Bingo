...
export default function LivePreview({ attendees, onClose }) {
  const canvasRef = useRef(null); const ballsRef = useRef(new Map());
  useEffect(() => {
    ...
    const tick = (now) => {
      const dt = Math.min((now - last) / 1000, .035); last = now; const width = canvas.clientWidth; const height = canvas.clientHeight; const radius = Math.max(22, Math.min(34, width / 11));
      attendees.forEach((person, index) => { if (!ballsRef.current.has(person.id)) ballsRef.current.set(person.id, { id: person.id, name: person.name, x: width * (.2 + ((hue(person.id) % 60) / 100)), y: -radius * (index + 2), vx: ((hue(person.id) % 100) - 50) * .35, vy: 0, color: palette[hue(person.id) % palette.length] }); });
      const balls = attendees.map(person => ballsRef.current.get(person.id)).filter(Boolean);
      balls.forEach(ball => { ball.vy += 620 * dt; ball.x += ball.vx * dt; ball.y += ball.vy * dt; if (ball.x - radius < 10) { ball.x = 10 + radius; ball.vx *= -.72; } if (ball.x + radius > width - 10) { ball.x = width - 10 - radius; ball.vx *= -.72; } if (ball.y + radius > height - 18) { ball.y = height - 18 - radius; ball.vy *= -.58; ball.vx *= .985; } });
      for (let i = 0; i < balls.length; i += 1) for (let j = i + 1; j < balls.length; j += 1) { const a = balls[i], b = balls[j]; const dx = b.x - a.x, dy = b.y - a.y, distance = Math.hypot(dx, dy) || 1; if (distance < radius * 2) { const nx = dx / distance, ny = dy / distance, overlap = radius * 2 - distance; a.x -= nx * overlap / 2; a.y -= ny * overlap / 2; b.x += nx * overlap / 2; b.y += ny * overlap / 2; const impulse = (b.vx - a.vx) * nx + (b.vy - a.vy) * ny; if (impulse < 0) { a.vx += impulse * nx; a.vy += impulse * ny; b.vx -= impulse * nx; b.vy -= impulse * ny; } } }
      ...
      balls.forEach(ball => { context.beginPath(); context.arc(ball.x, ball.y, radius, 0, Math.PI * 2); context.fillStyle = ball.color; context.fill(); context.fillStyle = '#fff'; context.font = `600 ${Math.max(12, radius * .47)}px system-ui`; context.textAlign = 'center'; context.textBaseline = 'middle'; context.fillText(initials(ball.name), ball.x, ball.y); }); frame = requestAnimationFrame(tick);
    };
