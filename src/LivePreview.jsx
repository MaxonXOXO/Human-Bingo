...
export default function LivePreview({ attendees, leaderboard = [], onClose }) {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      const dt = Math.min((now - last) / 1000, .035); last = now; const width = canvas.clientWidth; const height = canvas.clientHeight; const baseRadius = Math.max(22, Math.min(34, width / 11)); const progress = new Map(leaderboard.map(row => [row.attendee_id, row.total_cells ? Number(row.completed_count) / Number(row.total_cells) : 0]));
      attendees.forEach((person, index) => { if (!ballsRef.current.has(person.id)) ballsRef.current.set(person.id, { id: person.id, name: person.name, x: width * (.2 + ((hue(person.id) % 60) / 100)), y: -baseRadius * (index + 2), vx: ((hue(person.id) % 100) - 50) * .35, vy: 0, color: palette[hue(person.id) % palette.length], r: baseRadius }); });
